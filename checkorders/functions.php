...
function getUserDetails($usernamik, $tableName) {
        ...
        if ($result1->num_rows > 0) {
            while ($row1 = $result1->fetch_assoc()) {
                ...
                $user_telegramdeals  = $row1['user_telegramdeals'];
                $user_balance  = $row1['balance'];
                $fullDeposit = $row1['botdeposit'];
            }
        }
    ...
    function setPartialExOrder($order, $tableName, $totalqt, $ne, $api, $usernamik) {
      ...
      try {
          ...
          if ($orderstatus['executedQty'] > 0.00000001 && $orderstatus['executedQty'] !== $orderstatus['origQty']) {
              $newTotalQt = $totalqt - $orderstatus['executedQty'];
              $response = $api->cancel($symbol, $orderrr);
              $finalqua = number_format($newTotalQt, $ne, '.', '');
              $neworder = $api->sell($symbol, $finalqua, $price);
              $finalordersell = serialize($neworder);
              $sql_v   = "UPDATE `$tableName` SET totalqt = ?, orderprofit = ? WHERE username = '{$usernamik}';";
              $stmt    = $conn->prepare($sql_v);
              $stmt->bind_param("ss", $finalqua, $finalordersell);
              $stmt->execute();
          }
        }
      catch (Exception $e) {
    ...
    function calcBalance($usernamik, $finalprofit, $user_balance) {

      include "/var/www/mrpnl.com/myaccount/includes/dbh.inc.php";
      $newFinalProfit = $finalprofit * 0.15;
      $newFinalBalance = $user_balance - $newFinalProfit;

      $sql_v   = "UPDATE wptj_users SET balance = ? WHERE user_nicename = '{$usernamik}';";
      $stmt    = $conn->prepare($sql_v);
      $stmt->bind_param("s", $newFinalBalance);
      $stmt->execute();

    }

    function checkOrders($order, $i, $usernamik, $qt, $cp, $n, $ne, $on, $finalcoin, $api, $tableName, $actualtakeprofit) {
    ...
    function setDealCloseOrder($usernamik, $finalcoin, $api, $tableName) {
  ...
  function disableCancelledBot($nor, $usernamik) {
    ...
    function sendCancelledBot($nor, $tgchat, $usernamik) {
        if ($tgchat !== "0" && $nor == "0") {
    ...
    function getDealInformation($usernamik, $tableName) {
    ...
    function calculateTodayProfit($usernamik, $finalprofit) {
    ...
    function calculateDealNumber($usernamik) {
    ...
    function addHistoryRecord($usernamik, $finalprofit, $finalcoin, $dealNumber, $oz, $ordernumber, $execcprice) {
    ...
    function calcLiveAdd($usernamik, $finalprofit, $mydeposit, $tableName, $fullDeposit) {
